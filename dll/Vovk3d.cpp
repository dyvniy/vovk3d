...
IDirect3DTexture9*              g_pShadowMap = NULL;    // Texture to which the shadow map is rendered
IDirect3DSurface9*              g_pDSShadow = NULL;     // Depth-stencil buffer for rendering to shadow map
...
int fpe(int i){return 0;}
int fpr(double d, float f){return 0;}

func_pointer_empty beforeCreatePy	= fpe;
func_pointer_empty onCreatePy	= fpe;
func_pointer_empty onResetPy	= fpe;
func_pointer_render onMovePy	= fpr;
func_pointer_render onRenderPy	= fpr;
func_pointer_empty onLostPy		= fpe;
                                 ...
                                 void* pUserContext )
{
	...
	//{
	//	g_vwsMeshFile.push_back(g_aszMeshFile[i]);
	//	g_vmInitObjWorld.push_back(&g_amInitObjWorld[i]);
	//}
	...
	//float fm[] = {5.5f, 0.0f, 0.0f, 0.0f, // arrow1
	//				0.0f, 0.0f, 5.5f, 0.0f, 
	//				0.0f, -9.0f, 0.0f, 0.0f, 
	//				5.0f, 0.2f, 5.0f, 1.0f };
	//wchar_t wch[] = L"UI\\arrow.x";
	//addMesh(wch, fm);
	////g_vwsMeshFile.push_back(wch);
	////g_vmInitObjWorld.push_back(&m);
    ...
    WCHAR str[MAX_PATH];
    V_RETURN( DXUTFindDXSDKMediaFileCch( str, MAX_PATH, L"shaders/ShadowMap.fx" ) );
	...
	for( int i = 0; i < g_vwsMeshFile.size(); ++i )
    {
        ...
        g_Obj[i]->m_mWorld = *g_vmInitObjWorld[i];
    }
	...
	//func_pointer_empty 
	onCreatePy(g_destroyBeforeReset?1:0);
                                ...
                                const D3DSURFACE_DESC* pBackBufferSurfaceDesc, void* pUserContext )
{
    ...
    *( LPDWORD )lr.pBits = D3DCOLOR_RGBA( 255, 255, 255, 255 );
	...
	//func_pointer_empty 
	onResetPy(g_destroyBeforeReset?1:0);
	g_destroyBeforeReset = false;
...
{
	D3DXMATRIXA16 m;
    D3DXMatrixRotationY( &m, mul );
    D3DXMatrixMultiply( &g_Obj[idx]->m_mWorld, &g_Obj[idx]->m_mWorld, &m );
	return 0;
}
DLL_API int rotateAxis(int idx, float mul, float v[3])
{
	D3DXMATRIXA16 m;
    D3DXVECTOR3 vR( v );
    D3DXMatrixRotationAxis( &m, &vR, mul );
    D3DXMatrixMultiply( &g_Obj[idx]->m_mWorld, &m, &g_Obj[idx]->m_mWorld );
	return 0;
}
};
                  ...
                  const D3DXMATRIX* pmProj )
{
    ...
    if( SUCCEEDED( pd3dDevice->BeginScene() ) )
    {
        ...
        // Render the objects
		for( int obj = 0; obj < g_vwsMeshFile.size(); ++obj )
        {
...
{
	if (g_WindowClosed) return;
	if (g_destroyBeforeReset) return;
	...
	//func_pointer_empty 
	onDestroyPy(g_destroyBeforeReset?1:0);
}
